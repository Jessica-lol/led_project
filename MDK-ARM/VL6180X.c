...
uint8_t VL6180X_ReadByte(uint16_t reg)
{
	...
	uint8_t Index_L = (uint8_t)(reg & 0xff);
    I2C_Start(VL6180_I2C); 
	I2C_Send_Byte(VL6180_I2C,(VL6180X_DEFAULT_I2C_ADDR<<1)|0);//发送器件地址+写命令	
	I2C_Wait_Ack(VL6180_I2C);		//等待应答 
    I2C_Send_Byte(VL6180_I2C,Index_H);	//写寄存器地址
    I2C_Wait_Ack(VL6180_I2C);		//等待应答
	I2C_Send_Byte(VL6180_I2C,Index_L);	//写寄存器地址
	I2C_Wait_Ack(VL6180_I2C);	
	
    I2C_Start(VL6180_I2C);
	I2C_Send_Byte(VL6180_I2C,(VL6180X_DEFAULT_I2C_ADDR<<1)|1);//发送器件地址+读命令	
    I2C_Wait_Ack(VL6180_I2C);		//等待应答 
	res=I2C_Read_Byte(VL6180_I2C,0);//读取数据,发送nACK 
    I2C_Stop(VL6180_I2C);			//产生一个停止条件 
	return res;
}
...
uint8_t VL6180X_Read_HalfWold(uint16_t reg)
{
	...
	I2C_Send_Byte(VL6180_I2C,(VL6180X_DEFAULT_I2C_ADDR<<1)|1);//发送器件地址+读命令	
    I2C_Wait_Ack(VL6180_I2C);		//等待应答 
	res = I2C_Read_Byte(VL6180_I2C,1);//读取数据,发送ACK 
	res <<= 8;
	res |= I2C_Read_Byte(VL6180_I2C,0);//读取数据,发送nACK 
    I2C_Stop(VL6180_I2C);			//产生一个停止条件 
	return res;
